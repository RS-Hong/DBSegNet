...
# ======= Ablation-ready redefinitions (override) =======

class DualEncoder(nn.Module):
    def __init__(self, dims, depths_cnn, depths_tr, heads, sr, drop_path, use_ca, sr_kv,
                 fusion_mode: str = 'gated',   # 'gated' | 'sum'
                 branch: str = 'dual',         # 'dual' | 'cnn'
                 force_no_ca: bool = False):   # disable cross-attn even if enabled
        super().__init__()
        self.fusion_mode = fusion_mode
        self.branch = branch
        self.force_no_ca = force_no_ca

        # Stem：1/2 -> 1/4（两层 3×3,s2）
        self.stem = nn.Sequential(ConvBNAct(3, 32, 3,2,1), ConvBNAct(32, 64, 3,2,1))
    ...
    # fusion helper
    def _fuse(self, c, t, stage_idx: int):
        if self.fusion_mode == 'sum':
            return c + t
        lex = getattr(self, f'lex{stage_idx}')
        fu  = getattr(self, f'fu{stage_idx}')
        c, t = lex(c, t)
        return fu(c, t)
    ...
    def forward(self, x):
        ...
        # Default: dual branches
        c1 = self.c1(x); t1 = self.t1(x)
        f1 = self._fuse(c1, t1, 1)

        c2 = self.c2(f1); t2 = self.t2(t1)
        f2 = self._fuse(c2, t2, 2)

        c3 = self.c3(f2); t3 = self.t3(t2)
        if (self.ca3 is not None) and (not self.force_no_ca):
            t3 = t3 + self.ca3(t3, c3)
        f3 = self._fuse(c3, t3, 3)

        c4 = self.c4(f3); t4 = self.t4(t3)
        if (self.ca4 is not None) and (not self.force_no_ca):
            t4 = t4 + self.ca4(t4, c4)
        f4 = self._fuse(c4, t4, 4)

        return f0, f1, f2, f3, f4


class DBSegNet(nn.Module):
    def __init__(self, num_classes: int = 21, phi: str = 'cvmv_tiny_fast',
                 pretrained: bool = False, return_aux: bool = False,
                 fusion_mode: str = 'gated', branch: str = 'dual', force_no_ca: bool = False):
        super().__init__()
        assert phi in PHI, f"Unsupported phi: {phi}. Options: {list(PHI.keys())}"
        cfg = PHI[phi]
        self.return_aux = return_aux

        # same as original, with extra switches forwarded to backbone
        self.backbone = DualEncoder(cfg['dims'], cfg['depths_cnn'], cfg['depths_tr'],
                                    cfg['heads'], cfg['sr'], cfg['drop_path'],
                                    cfg['use_ca'], cfg['sr_kv'],
                                    fusion_mode=fusion_mode, branch=branch, force_no_ca=force_no_ca)
        self.decode = SegFormerDecoderHead(list(cfg['dims']), cfg['decoder_dim'], num_classes)
        if self.return_aux:
            self.aux0 = AuxHead(64, num_classes)
            self.aux3 = AuxHead(cfg['dims'][2], num_classes)
