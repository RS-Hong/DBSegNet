...
class DualEncoder(nn.Module):
    def __init__(self, dims, depths_cnn, depths_tr, heads, sr, drop_path, use_ca, sr_kv, fusion_mode='gate', branch='dual', force_no_ca=False):
        super().__init__()
...
class DBSegNet(nn.Module):
    def __init__(self, num_classes: int = 21, phi: str = 'cvmv_tiny_fast',
        ...
        assert branch in ['dual', 'cnn', 'tr'], f"Unsupported branch: {branch}"
        assert fusion_mode in ['gated', 'sum', 'gate', 'agg'], f"Unsupported fusion_mode: {fusion_mode}"
        cfg = PHI[phi]
        self.return_aux = return_aux

        fusion_type = 'agg' if fusion_mode == 'agg' else ('gate' if fusion_mode in ['gated', 'gate'] else fusion_mode)
        self.backbone = DualEncoder(cfg['dims'], cfg['depths_cnn'], cfg['depths_tr'],
                                    cfg['heads'], cfg['sr'], cfg['drop_path'],
                                    cfg['use_ca'], cfg['sr_kv'],
                                    fusion_type, branch, force_no_ca)
...
if __name__ == '__main__':
    m = DBSegNet(num_classes=2, phi='cvmv_tiny_fast', branch='dual', fusion_mode='agg')
